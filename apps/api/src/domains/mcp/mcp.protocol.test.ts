...
describe('handleMcpRequest', () => {
    ...
    it('initialize returns the protocol version, capabilities and serverInfo', async () => {
        const r = await mod.handleMcpRequest({jsonrpc: '2.0', id: 1, method: 'initialize'}, []);
    ...
    it('ping returns an empty result', async () => {
        const r = await mod.handleMcpRequest({id: 2, method: 'ping'}, []);
        expect(r).toMatchObject({id: 2, result: {}});
    });

    it('tools/list returns the advertised tools', async () => {
        const r = await mod.handleMcpRequest({id: 3, method: 'tools/list'}, []);
        expect((r as {result: {tools: unknown[]}}).result.tools).toHaveLength(1);
    });

    it('tools/call dispatches to runToolCall with the caller integrationIds', async () => {
        (tools.runToolCall as unknown as ReturnType<typeof vi.fn>).mockResolvedValue({content: [{type: 'text', text: 'ok'}]});
        const r = await mod.handleMcpRequest({id: 4, method: 'tools/call', params: {name: 'run_sql', arguments: {sql: 'SELECT 1'}}}, ['i1']);
        expect(tools.runToolCall).toHaveBeenCalledWith('run_sql', {sql: 'SELECT 1'}, ['i1']);
    ...
    it('tools/call reports a tool failure in-band (isError), not as a JSON-RPC error', async () => {
        ...
        const r = await mod.handleMcpRequest({id: 5, method: 'tools/call', params: {name: 'run_sql', arguments: {}}}, []);
    ...
    it('returns null for a notification (no id)', async () => {
        expect(await mod.handleMcpRequest({method: 'notifications/initialized'}, [])).toBeNull();
    });

    it('unknown request method → -32601', async () => {
        const r = await mod.handleMcpRequest({id: 6, method: 'foo/bar'}, []);
        expect((r as {error: {code: number}}).error.code).toBe(-32601);
    });

    it('missing method on a request → -32600', async () => {
        const r = await mod.handleMcpRequest({id: 7}, []);
